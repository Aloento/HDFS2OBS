...
class OBSListing {
     ...
     * {@link OBSFileSystem#listStatus(Path)}
    ...
    static final class SingleStatusRemoteIterator implements RemoteIterator<LocatedFileStatus> {
         ...
         * {@inheritDoc}
         *
         * @return true if there is a file status to return: this is always
         * false for the second iteration, and may be false for the first.
         */
        @Override
         ...
         *                                the first call and a null {@link
         *                                LocatedFileStatus} entry was passed to
         *                                the constructor.
         */
        @Override
     ...
     * there are more entries remaining. If there are no more results remaining
     * in the (filtered) results of the current listing request, then another
     * request is made
     ...
     * declaring that if the {@link ObjectListingIterator#hasNext()} is true
     * then there are more results. Instead the next batch of results must be
     ...
     * #hasNext()} during its operation. This is critical to ensure that a
     * listing obtained through a sequence of {@link #next()} will complete with
     * the same set of results as a classic {@code while(it.hasNext()} loop.
     *
    ...
    class FileStatusListingIterator implements RemoteIterator<FileStatus> {
         ...
         *
         * @param listPath           the listing iterator from a listObjects
         *                           call.
         * @param pathFilter         the filter on which paths to accept
         * @param fileStatusAcceptor the class/predicate to decide which entries
         *                           to accept in the listing based on the full
         *                           file status.
         * @throws IOException IO Problems
         */
        ...
        /**
         * Try to retrieve another batch. Note that for the initial batch,
         * {@link ObjectListingIterator} does not generate a request; it simply
    ...
    /**
     * Wraps up OBS `ListObjects` requests in a remote iterator which will ask
     * for more listing data if needed.
     *
     * <p>That is:
     *
     ...
     * batch of results.
     *
     * <p>3. The {@link #hasNext()} predicate returns true for the initial
     * call, where {@link #next()} will return the initial results. It declares
    ...
    class ObjectListingIterator implements RemoteIterator<ObjectListing> {
        ...
        /**
         * Constructor -calls {@link OBSCommonUtils#listObjects} on the request
         * to populate the initial set of results/fail if there was a problem
